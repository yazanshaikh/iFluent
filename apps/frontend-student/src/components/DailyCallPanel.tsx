 ...
 * Shows the teacher (remote) full-bleed with the student (local) as a small PiP,
 ...
 * rebuilt (expo run:android / eas build) after adding the SDK. Running new JS
...
// auto-extract it from the URL (unlike the web prebuilt), so split it out and
// pass it explicitly to join({ url, token }) — otherwise a private room rejects
// with "You are not allowed to join this meeting".
...
export function DailyCallPanel({ roomUrl }: Props) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setFatal('تعذّر تشغيل محرّك الفيديو. تأكد من إعادة بناء التطبيق.');
      return;
    }
    ...
    call.join(token ? { url, token } : { url }).catch((e: any) => {
      ...
      const raw = e?.message || e?.errorMsg || JSON.stringify(e)?.slice(0, 140) || 'empty';
      setFatal(`CATCH: ${raw}\n[${diag}]`);
    });
        ...
        <View style={s.placeholder}>
          {fatal ? (
            ...
            <>
              ...
              <Text style={s.placeholderTxt}>في انتظار انضمام المعلم…</Text>
            </>
          ) : (
            <>
              <ActivityIndicator color="#10b981" size="large" />
              <Text style={s.placeholderTxt}>جاري الاتصال…</Text>
            </>
          )}
        </View>
      )}
